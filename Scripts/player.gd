...
var senstivity = 0.008 #Mouse senstivity
...
func _unhandled_input(event): #Used for unhandled inputs
...
func _physics_process(delta): #Phyisics function (locked to 60fps)
	...
	player_direction = (head.transform.basis * Vector3(inputs.x, 0, inputs.y)) #Assigns the inputs vector to the player_direction vector based on the head direction
